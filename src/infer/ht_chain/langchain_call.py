...
def get_langchain_completion(logger, clargs):
    ...
    if clargs['syn_type'] == 'origin':  # if hints is None, it is a origin code generation task
        ...
        user_template = """/
                ...
                {signature}
                ```
                Write a solution with ```{language}``` tag.
                """
        ...
        user_template = """/
                ...
                {hints}
                ```
                Write a solution with ```{language}``` tag, please using hints to help you think about the solution. 
        """
        text = ({"content": clargs["content"], "signature": clargs["signature"], 'language': clargs['language'],
                 "hints": clargs["hints"]})
        ...
        user_template = """/
            ...
            """
        text = ({"content": clargs["content"], "signature": clargs["signature"], 'language': clargs['language']})
    elif clargs['syn_type'] == 'step_template':
        system_template = """
            You are a helpful assistant to generate code.
            """
        # user_template = "{content} {signature} {hints}"
        user_template = """/
            ...
            {signature}
            ```
            For this above question, write a solution with ```{language}``` tag. 
        ...
        text = ({"content": clargs["content"], "signature": clargs["signature"], 'language': clargs['language'],
                 "hints": clargs["hints"]})
    elif clargs['syn_type'] == 'step_chain':
        system_template = """
            You are a helpful assistant to generate code.   
            """
        # user_template = "{content} {signature} {hints}"
        user_template = """/   
            ...
            {hints}
            ```
            Write a solution with ```{language}``` tag, please using hints to help you think about the solution. Let's think step by step.
        """
        text = ({"content": clargs["content"], "signature": clargs["signature"], 'language': clargs['language'],
                 "hints": clargs["hints"]})
    else:
