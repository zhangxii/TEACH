...
def print_results(results: Dict, args:argparse.Namespace=None):
    """
    Given the data evaluated against the testcases we output some statistics.
...
def eval_and_save_problems(args):
    ...
    for index, problem in enumerate(tqdm(problems)):
        ...
        if args.debug:
            print(f"\nHow to read data [-2] = compile error, [-1] = runtime error, [False] = failed test case, [True] = passed test case")
            #print(f"data = {res}")
...
def main(args):
    ...
    if args.print_results:
        ...
        if os.path.exists(results_loc):
            ...
            print("No data to print exiting.")
            exit()
...
if __name__ == "__main__":
    ...
    parser.add_argument("-p", "--print_results", action="store_true", help="If you have already evaluated the data and only want to print them.")
    parser.add_argument("--skip_evals", action="store_true", help="If you want to skip the evals similar to print data.")
    parser.add_argument("-d", "--debug", action="store_true")
    parser.add_argument("--save", type=str, default="./data", help="Where the evaluated data is loaded from and data saved to.")
